...
class VaporSong:
	...
	def vaporize_song(bandname,fname):
		...
		VaporSong.slow_down(fname, 0.7, "samp/out.wav")
		# Randomly crop to a minute of the song
		print "~Cropping"
		VaporSong.random_crop("samp/out.wav",60,"samp/outcrop.wav")
		# Get a list of beats
		print "~Doing Beat Analysis"
		bm = VaporSong.fetchbeats("samp/outcrop.wav")
		# Split into an array of strings repping beats
		print "~Split into beats"
		splitd = VaporSong.split("samp/outcrop.wav",bm)
		...
		VaporSong.combine(sectionflat,"samp/out_norev.wav")
		os.system("mkdir " + bandname)
		VaporSong.reverbize("samp/out_norev.wav",bandname+"/"+fname.split("/")[1]+".wav")
		os.system("rm samp/*")
